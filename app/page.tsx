...
const content = {
  en: {
    ...
    frameworkSubtitle: "Simple, clean, and built to improve",
    frameworkItems: [
    ...
    sectors: ["Logistics", "Construction", "Energy", "Industrial", "Service businesses", "Modern B2B"],
    ...
    nextStepTitle: "Want to see what Travon could improve first?",
    ctaEyebrow: "Contact",
  ...
  nl: {
    ...
    frameworkTitle: "Travon Framework",
    frameworkSubtitle: "Simpel, strak en gebouwd om te verbeteren",
    frameworkItems: [
    ...
    sectors: ["Logistiek", "Bouw", "Energie", "Industrie", "Servicebedrijven", "Modern B2B"],
    ...
    nextStepTitle: "Wil je zien wat Travon als eerste kan verbeteren?",
    ctaEyebrow: "Contact",
...
function SectionTitle({ eyebrow, title, text }: { eyebrow: string; title: string; text: string }) {
  return (
    <div className="max-w-3xl">
      <div className="text-sm font-semibold uppercase tracking-[0.18em] text-[#17795E]">{eyebrow}</div>
...
export default function Page() {
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-[#F7FAF8] text-[#0F1720]">
        ...
        >
          <div className="flex items-center justify-between px-5 py-4 lg:px-6">
            ...
            <nav className="hidden items-center gap-8 lg:flex">
              {navItems.map((item) => (
                <a key={item.href} href={item.href} className="text-sm font-medium text-[#55616D] transition hover:text-[#17795E]">
                  {item.label}
                </a>
              ))}
            </nav>
            ...
            <div className="hidden items-center gap-3 lg:flex">
              <div className="inline-flex rounded-xl border border-[#DDE7E0] bg-[#FBFDFC] p-1 shadow-sm">
                <button
                  onClick={() => setLang("en")}
                  className={`rounded-lg px-3 py-2 text-sm font-medium transition ${lang === "en" ? "bg-[linear-gradient(135deg,#1F8A70_0%,#179A74_100%)] text-white shadow-sm" : "text-[#55616D]"}`}
                >
                  EN
                </button>
                <button
                  onClick={() => setLang("nl")}
                  className={`rounded-lg px-3 py-2 text-sm font-medium transition ${lang === "nl" ? "bg-[linear-gradient(135deg,#1F8A70_0%,#179A74_100%)] text-white shadow-sm" : "text-[#55616D]"}`}
                >
                  NL
                </button>
              </div>
        ...
        <div className="grid items-center gap-14 lg:grid-cols-[1.08fr_0.92fr]">
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }}>
            ...
            <div className="mt-12 flex flex-wrap items-center gap-4 text-sm text-[#6B7280]">
              <span>{lang === "en" ? "Based in Rotterdam" : "Gebaseerd in Rotterdam"}</span>
              <span className="h-1.5 w-1.5 rounded-full bg-[#1F8A70]" />
              <span>{lang === "en" ? "Websites" : "Websites"}</span>
              <span className="h-1.5 w-1.5 rounded-full bg-[#1F8A70]" />
              <span>{lang === "en" ? "Automation" : "Automatisering"}</span>
              <span className="h-1.5 w-1.5 rounded-full bg-[#1F8A70]" />
              <span>{lang === "en" ? "Digital systems" : "Digitale systemen"}</span>
            </div>

            <div className="mt-12 grid gap-4 sm:grid-cols-3">
              {t.metrics.map(([value, label]) => (
                <div key={label} className="rounded-2xl border border-[#DDE7E0] bg-white/95 p-5 shadow-[0_10px_28px_rgba(15,23,32,0.05)] backdrop-blur-sm">
                  <div className="text-2xl font-semibold tracking-[-0.02em] text-[#17795E]">{value}</div>
            ...
            <div className="relative overflow-hidden rounded-[28px] border border-[#DDE7E0] bg-[linear-gradient(180deg,#FFFFFF_0%,#F7FCF9_100%)] p-7 shadow-[0_20px_60px_rgba(15,23,32,0.08)] md:p-8">
              ...
              <div className="mt-8 space-y-4">
                {t.frameworkItems.map((item) => (
                  <div key={item} className="group rounded-2xl border border-[#E5ECE7] bg-white p-4 transition hover:border-[#CFE3D8] hover:shadow-[0_10px_24px_rgba(15,23,32,0.05)]">
      ...
      <section id="problems" className="relative z-10 bg-[linear-gradient(180deg,#EEF8F3_0%,#F7FBF9_100%)]">
          ...
          <div className="mt-12 grid gap-6 md:grid-cols-2 lg:grid-cols-4">
            {t.problems.map((problem, index) => (
              <motion.div
                ...
                className="rounded-[24px] border border-[#D7E8DF] bg-white p-6 shadow-[0_8px_24px_rgba(15,23,32,0.04)] transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_20px_40px_rgba(15,23,32,0.08)]"
              >
                <div className="mb-5 inline-flex h-10 w-10 items-center justify-center rounded-xl bg-[linear-gradient(135deg,#EAF7F1_0%,#F8FDFA_100%)] text-sm font-semibold text-[#17795E]">
                  0{index + 1}
                </div>
      ...
      <section id="why" className="relative z-10 bg-[linear-gradient(180deg,#FBFCFB_0%,#F5FAF7_100%)]">
        <div className="mx-auto max-w-7xl px-6 py-28 md:px-10 lg:px-12">
          ...
          <div className="mt-12 grid gap-6 lg:grid-cols-3">
            {t.reasons.map((reason, index) => (
              <motion.div
                ...
                viewport={{ once: true, amount: 0.2 }}
                transition={{ duration: 0.45, delay: index * 0.08 }}
                className="rounded-[28px] border border-[#DDE7E0] bg-white p-7 shadow-[0_10px_24px_rgba(15,23,32,0.04)] transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_20px_40px_rgba(15,23,32,0.07)]"
              >
            ...
            {t.process.map((step, index) => (
              ...
              >
                <div className="flex items-start gap-4">
                  <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-[linear-gradient(135deg,#EAF7F1_0%,#F8FDFA_100%)] text-sm font-semibold text-[#17795E]">
                    0{index + 1}
                  </div>
                  <div>
      ...
      <section className="relative z-10 bg-[linear-gradient(180deg,#EAF7F1_0%,#F8FCFA_100%)]">
        <div className="mx-auto max-w-7xl px-6 py-28 md:px-10 lg:px-12">
          ...
          <div className="mt-12 grid gap-6 lg:grid-cols-3">
            {t.examples.map((item, index) => (
              <motion.div
                ...
                viewport={{ once: true, amount: 0.2 }}
                transition={{ duration: 0.45, delay: index * 0.08 }}
                className="rounded-[28px] border border-[#D7E8DF] bg-white p-7 shadow-[0_10px_24px_rgba(15,23,32,0.04)] transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_20px_40px_rgba(15,23,32,0.08)]"
              >
                <div className="text-sm font-semibold text-[#17795E]">{item.number}</div>
      ...
      <section id="pricing" className="relative z-10 bg-[linear-gradient(180deg,#FBFCFB_0%,#F5FAF7_100%)]">
        <div className="mx-auto max-w-7xl px-6 py-28 md:px-10 lg:px-12">
          ...
          <div className="mt-12 grid gap-6 lg:grid-cols-3">
            {t.pricingCards.map((item, index) => (
              <motion.div
                ...
                viewport={{ once: true, amount: 0.2 }}
                transition={{ duration: 0.45, delay: index * 0.08 }}
                className={`rounded-[28px] border p-7 shadow-[0_12px_30px_rgba(15,23,32,0.04)] transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_20px_40px_rgba(15,23,32,0.08)] ${index === 1 ? "border-[#CFE3D8] bg-[linear-gradient(180deg,#FFFFFF_0%,#ECF8F2_100%)]" : "border-[#DDE7E0] bg-white"}`}
              >
                <h3 className="text-2xl font-semibold tracking-[-0.02em] text-[#0F1720]">{item.title}</h3>
                <div className="mt-3 text-3xl font-semibold tracking-[-0.03em] text-[#17795E]">{item.price}</div>
      ...
      <section className="relative z-10 mx-auto max-w-7xl px-6 py-10 md:px-10 lg:px-12">
        <div className="rounded-[28px] border border-[#DDE7E0] bg-[linear-gradient(135deg,#FFFFFF_0%,#F2FAF6_100%)] p-7 shadow-[0_14px_34px_rgba(15,23,32,0.05)] md:flex md:items-center md:justify-between">
          <div>
            <div className="text-sm font-semibold uppercase tracking-[0.18em] text-[#17795E]">{t.nextStepEyebrow}</div>
          ...
          <div className="relative grid gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">
            ...
            <div className="rounded-[28px] border border-white/10 bg-white/8 p-6 backdrop-blur-md">
              <div className="space-y-4">
                ...
                <a href="tel:+310636490512" className="inline-flex w-full items-center justify-center rounded-xl border border-white/15 bg-white/8 px-6 py-4 text-base font-medium text-white transition hover:bg-white/14">
                  {t.ctaSecondary}
                </a>
              </div>
      ...
      <footer className="relative z-10 border-t border-[#E2EAE4] bg-white/85">
        <div className="mx-auto grid max-w-7xl gap-10 px-6 py-12 md:grid-cols-[1.2fr_0.8fr_0.8fr] md:px-10 lg:px-12">
          ...
          <div>
            <div className="text-sm font-semibold uppercase tracking-[0.14em] text-[#17795E]">{t.footerMenu}</div>
            <div className="mt-4 space-y-3 text-sm text-[#55616D]">
              <a href="#services" className="block transition hover:text-[#17795E]">{t.nav.services}</a>
              <a href="#process" className="block transition hover:text-[#17795E]">{t.nav.process}</a>
              <a href="#pricing" className="block transition hover:text-[#17795E]">{t.nav.pricing}</a>
              <a href="#contact" className="block transition hover:text-[#17795E]">{t.nav.contact}</a>
            </div>
          </div>
          <div>
            <div className="text-sm font-semibold uppercase tracking-[0.14em] text-[#17795E]">{t.footerContact}</div>
            <div className="mt-4 space-y-3 text-sm text-[#55616D]">
